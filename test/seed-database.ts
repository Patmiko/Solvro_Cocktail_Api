...
export async function seedDatabase() {
  // --- Cocktail Categories ---
  const classic = await prisma.cocktailCategory.create({
    data: { name: "Classic" },
  });
  const tropical = await prisma.cocktailCategory.create({
    data: { name: "Tropical" },
  });

  // --- Ingredient Types ---
  const spirit = await prisma.ingredientType.create({
    data: { name: "Spirit" },
  });
  ...
  const garnish = await prisma.ingredientType.create({
    data: { name: "Garnish" },
  });
