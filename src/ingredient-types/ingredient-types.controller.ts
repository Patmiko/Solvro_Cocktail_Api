...
export class IngredientTypesController {
  ...
  @ApiResponse({ status: 201, description: 'Ingredient type successfully created.' })
  ...
  @ApiResponse({ status: 200, description: 'List of all ingredient types returned.' })
  async findAll() {
    return await this.ingredientTypesService.findAll();
  }

  @Get(':name')
  @ApiOperation({ summary: 'Get a specific ingredient type by ID' })
  ...
  @ApiResponse({ status: 200, description: 'Ingredient type successfully updated.' })
  @ApiResponse({ status: 404, description: 'Ingredient type not found.' })
  @UseGuards(AuthGuard,RoleGuard)
  @Roles(Role.ADMIN, Role.MODERATOR)
  @ApiBearerAuth("access-token")
  async update(@Param('name') name: string, @Body() updateIngredientTypeDto: UpdateIngredientTypeDto) {
    return await this.ingredientTypesService.update(name, updateIngredientTypeDto);
  }

  @Delete(':name')
  @ApiOperation({ summary: 'Delete an ingredient type by ID' })
  @ApiResponse({ status: 200, description: 'Ingredient type successfully deleted.' })
  @ApiResponse({ status: 404, description: 'Ingredient type not found.' })
  @UseGuards(AuthGuard,RoleGuard)
  @Roles(Role.ADMIN, Role.MODERATOR)
  @ApiBearerAuth("access-token")
