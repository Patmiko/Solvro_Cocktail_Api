...
export class RatingsController {
  ...
  @ApiResponse({
    status: 409,
    description: "You have already rated this cocktail.",
  })
  @UseGuards(AuthGuard)
  ...
  @ApiParam({ name: "cocktailId", type: Number })
  async findAll(@Param("cocktailId") cocktailId: string) {
  ...
  @ApiResponse({
    status: 200,
    description: "The rating has been successfully fetched.",
  })
  ...
  async update(
    ...
    return await this.ratingsService.update(
      +cocktailId,
      updateRatingDto,
      userEmail,
      request.user,
  ...
  async remove(
    ...
    return await this.ratingsService.remove(
      +cocktailId,
      userEmail,
      request.user,
    );
